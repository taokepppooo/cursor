...
var _chunkCDGJIMJGcjs = require('./chunk-CDGJIMJG.cjs');
...
function hideStackFrames(wrappedFunction) {
  const hidden2 = nodeInternalPrefix + wrappedFunction.name;
  Object.defineProperty(wrappedFunction, "name", { value: hidden2 });
  ...
  if (string !== void 0) {
    ...
    try {
      ...
      const error = new ERR_INVALID_PACKAGE_CONFIG$1(
        jsonPath,
        (base ? `"${specifier}" from ` : "") + _url.fileURLToPath.call(void 0, base || specifier),
        cause.message
      );
      error.cause = cause;
      throw error;
...
function getPackageScopeConfig(resolved) {
  ...
  while (true) {
    ...
    const packageConfig = read(_url.fileURLToPath.call(void 0, packageJSONUrl), {
      specifier: resolved
    });
  ...
  const packageJSONPath = _url.fileURLToPath.call(void 0, packageJSONUrl);
  return {
...
function emitInvalidSegmentDeprecation(target, request, match, packageJsonUrl, internal, base, isTarget) {
  ...
  _process3.default.emitWarning(
    `Use of deprecated ${double ? "double slash" : "leading or trailing slash matching"} resolving "${target}" for module request "${request}" ${request === match ? "" : `matched to "${match}" `}in the "${internal ? "imports" : "exports"}" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${_url.fileURLToPath.call(void 0, base)}` : ""}.`,
    "DeprecationWarning",
    "DEP0166"
  );
}
...
function tryStatSync(path4) {
  try {
    return _fs.statSync.call(void 0, path4);
  } catch (e) {
  }
}
    ...
    while (++i < keys.length) {
      ...
      if (isArrayIndex(key)) {
        throw new ERR_INVALID_PACKAGE_CONFIG(
          _url.fileURLToPath.call(void 0, packageJsonUrl),
          base,
...
function emitTrailingSlashPatternDeprecation(match, pjsonUrl, base) {
  ...
  _process3.default.emitWarning(
    `Use of deprecated trailing slash pattern mapping "${match}" in the "exports" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${_url.fileURLToPath.call(void 0, base)}` : ""}. Mapping specifiers ending in "/" is no longer supported.`,
    "DeprecationWarning",
    "DEP0155"
  );
}
...
function packageImportsResolve(name, base, conditions) {
  if (name === "#" || name.startsWith("#/") || name.endsWith("/")) {
    ...
    throw new ERR_INVALID_MODULE_SPECIFIER(name, reason, _url.fileURLToPath.call(void 0, base));
  }
  let packageJsonUrl;
...
function packageResolve(specifier, base, conditions) {
  ...
  const packageConfig = getPackageScopeConfig(base);
  if (packageConfig.exists) {
    const packageJsonUrl2 = _url.pathToFileURL.call(void 0, packageConfig.pjsonPath);
...
function _resolve(id, options2 = {}) {
  ...
  if (_chunkTR37ZOITcjs.isAbsolute.call(void 0, id)) {
    try {
  ...
  const conditionsSet = options2.conditions ? new Set(options2.conditions) : DEFAULT_CONDITIONS_SET;
  const _urls = (Array.isArray(options2.url) ? options2.url : [options2.url]).filter(Boolean).map((url) => new URL(normalizeid(url.toString())));
  if (_urls.length === 0) {
  ...
  for (const url of _urls) {
    if (url.protocol === "file:") {
      urls.push(
        new URL("./", url),
        // If url is directory
        new URL(_chunkCDGJIMJGcjs.joinURL.call(void 0, url.pathname, "_index.js"), url),
  ...
  for (const url of urls) {
    ...
    for (const prefix of ["", "/index"]) {
      for (const extension of options2.extensions || DEFAULT_EXTENSIONS) {
        resolved = _tryModuleResolve(
          _chunkCDGJIMJGcjs.joinURL.call(void 0, id, prefix) + extension,
          url,
          conditionsSet
        );
        if (resolved) {
...
function resolveSync(id, options2) {
  return _resolve(id, options2);
}
function resolvePathSync(id, options2) {
  return fileURLToPath(resolveSync(id, options2));
}
function resolvePath(id, options2) {
  try {
    return Promise.resolve(resolvePathSync(id, options2));
  } catch (error) {
...
async function importModule(path4) {
  const i = await Promise.resolve().then(() => _interopRequireWildcard(require(path4)));
  if (i)
...
async function detect(options2 = {}) {
  const { cwd, onUnknown } = options2;
  for (const directory of lookup(cwd)) {
    for (const lock of Object.keys(LOCKS)) {
      if (await fileExists2(_path2.default.join(directory, lock))) {
        const name = LOCKS[lock];
        const result2 = await parsePackageJson(_path2.default.join(directory, "package.json"), onUnknown);
        if (result2)
          return result2;
        else
    ...
    const result = await parsePackageJson(_path2.default.join(directory, "package.json"), onUnknown);
    if (result)
      return result;
  }
  return null;
}
function* lookup(cwd = _process3.default.cwd()) {
  let directory = _path2.default.resolve(cwd);
...
function handlePackageManager(filepath, onUnknown) {
  var _a;
  try {
    const pkg = JSON.parse(_fs2.default.readFileSync(filepath, "utf8"));
    let agent;
    if (typeof pkg.packageManager === "string") {
      ...
      if (name === "yarn" && Number.parseInt(ver) > 1) {
        ...
        return (_a = onUnknown == null ? void 0 : onUnknown(pkg.packageManager)) != null ? _a : null;
      }
    }
  } catch (e) {
  }
...
var import_meta = {};
var require2 = _module.createRequire.call(void 0, import_meta.url);
...
function Ft(t, e) {
  let n = e.value.split(_path.delimiter), r = t, s;
  do
    n.push(_path.resolve.call(void 0, r, "node_modules", ".bin")), s = r, r = _path.dirname.call(void 0, r);
  while (r !== s);
  return { key: e.key, value: n.join(_path.delimiter) };
}
function I(t, e) {
  let n = {
    ...process.env,
...
var L = (t) => {
  let e = t.length, n = new (0, _stream.PassThrough)(), r = () => {
    --e === 0 && n.emit("end");
  };
  for (let s of t)
...
function we(t, e) {
  return {
    command: _path.normalize.call(void 0, t),
    args: e != null ? e : []
  };
}
function xe(t) {
...
var R = class {
  constructor(e, n, r) {
    ...
    _chunk42PBKAQTcjs.__publicField.call(void 0, this, "_onError", (e) => {
    ...
    this._options = {
      ...ge,
      ...r
    }, this._command = e, this._args = n != null ? n : [], this._processClosed = new Promise((s) => {
  ...
  async *[Symbol.asyncIterator]() {
    ...
    let r = L(n), s = _readline2.default.createInterface({
      input: r
    });
  ...
  spawn() {
    let e = _process2.cwd.call(void 0, ), n = this._options, r = {
      ...Ee,
      ...n.nodeOptions
    }, s = [];
    ...
    let { command: o, args: i } = we(this._command, this._args), a = (0, Pt._parse)(o, i, r), c = _child_process.spawn.call(void 0, 
      a.command,
      a.args,
      a.options
    );
...
// ../../node_modules/.pnpm/@antfu+install-pkg@1.0.0/node_modules/@antfu/install-pkg/dist/index.js
async function detectPackageManager(cwd = _process3.default.cwd()) {
...
async function installPackage(names, options2 = {}) {
  ...
  if (!Array.isArray(names))
    names = [names];
  const args = (typeof options2.additionalArgs === "function" ? options2.additionalArgs(agent, detectedAgent) : options2.additionalArgs) || [];
  ...
  if (agent === "pnpm" && _fs.existsSync.call(void 0, _path.resolve.call(void 0, (_a = options2.cwd) != null ? _a : _process3.default.cwd(), "pnpm-workspace.yaml"))) {
    args.unshift(
      "-w",
      /**
...
async function tryInstallPkg(name, autoInstall) {
  ...
  if (!tasks[name]) {
    ...
    if (typeof autoInstall === "function") {
      tasks[name] = pending = autoInstall(name).then(() => _chunkTR37ZOITcjs.sleep.call(void 0, 300)).finally(() => {
        pending = void 0;
      });
    } else {
      tasks[name] = pending = installPackage(name, {
        dev: true,
        preferOffline: true
      }).then(() => _chunkTR37ZOITcjs.sleep.call(void 0, 300)).catch((e) => {
...
async function loadCollectionFromFS(name, autoInstall = false, scope = "@iconify-json", cwd = process.cwd()) {
  ...
  async function task() {
    ...
    let stat;
    try {
      stat = jsonPath ? await _fs.promises.lstat(jsonPath) : void 0;
    } catch (err) {
      return void 0;
    }
    if (stat == null ? void 0 : stat.isFile()) {
      return JSON.parse(
        await _fs.promises.readFile(jsonPath, "utf8")
      );
    } else {
      return void 0;
    }
...
var loadNodeIcon = async (collection, icon, options2) => {
  let result = await _chunkTR37ZOITcjs.loadIcon.call(void 0, collection, icon, options2);
  if (result) {
    return result;
  }
  const cwds = Array.isArray(options2 == null ? void 0 : options2.cwd) ? options2.cwd : [options2 == null ? void 0 : options2.cwd];
  for (let i = 0; i < cwds.length; i++) {
    const iconSet = await loadCollectionFromFS(
      collection,
      i === cwds.length - 1 ? options2 == null ? void 0 : options2.autoInstall : false,
      void 0,
      cwds[i]
    );
    if (iconSet) {
      result = await _chunkTR37ZOITcjs.searchForIcon.call(void 0, 
        iconSet,
        collection,
        _chunkTR37ZOITcjs.getPossibleIconNames.call(void 0, icon),
        options2
      );
      if (result) {
        return result;
  ...
  if (options2 == null ? void 0 : options2.warn) {
