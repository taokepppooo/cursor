...
// open add mark panel
function openNewPanel(issueText, startLine, endLine) {
    const markWebViewPanel = vscode.window.createWebviewPanel('markdown.preview', 'Mark Bad Code', { preserveFocus: false, viewColumn: -2 }, { enableCommandUris: true, enableScripts: true, enableFindWidget: true });
...
function hightlightCodes(startPos, endPos) {
...
function updateFile(type, noteTexts) {
    let storeString = '';
    // read the file first
...
function openTheFile() {
    fs.readFile(_storeFile.path, (error, uint8Arr) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (strArray[0] === 'unresolved') {
            const doc = yield vscode.workspace.openTextDocument(vscode.Uri.parse('file:' + filePath));
...
function _getHtmlForWebview(webview, issueText, startLine, endLine) {
